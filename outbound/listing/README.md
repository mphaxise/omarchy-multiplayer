...
**Twenty-seven commands.** All start with `omarchy-agent-session-`. The main ones are `new`, `list`, `open`, `send`, `stop`, `done`, `receipt`, and `show`; `add` and `lanes` are the lane ones. The rest handle names, goals, modes, previews, captures, artifacts, watchers, and who may see or suggest on a session.
