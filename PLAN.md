...
**Keepalive maintenance.** The dev repository is the source; `outbound/build-listing.sh` assembles the listing repository, which is what the rig installs and what users get. Versions follow the manifest: 0.1.0 shipped; 0.2.0 after 2a; 0.3.0 after slice 2's review. A bug reported against the listing is reproduced on the rig, fixed in the dev repository with a test, and shipped through the same build, in that order.
