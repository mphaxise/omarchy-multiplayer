...
Receipts are never deleted by anything in slice 1. `session.json`, `events.jsonl`, `receipt.json`, and `artifacts/` persist under `~/.local/state/omarchy/sessions/<id>/` for as long as the directory exists, and invariant 7 in `01-session-model.md` guarantees no slice-1 command removes it. A `sessions prune` command is reserved: the name is claimed so a later slice can add retention without a naming collision, but it ships with no implementation and no schedule in slice 1.
