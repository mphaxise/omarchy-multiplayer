...
## Addendum, 21:00 to 21:40: the closed loop, from the terminal and from the panel

Run 3 (`findings/closed-loop.md`, `captures/evaluation-run3-closed-loop_hands-on_arm-port_2026-09-02/`) ran the spec/09 loop over ssh: session `loop-hello` on a worktree of the spike repo, a three-line intent, a file-URL preview in a Chromium app window, three captures, three instructions each sent `--about` the capture that prompted it, three commits, `done --verdict kept`. It found the `--until idle` wait matching neither of Herdr's `done` and `idle`, `done` leaving the harness running, and the loop view too thin; all three fixed the same evening.

Run 4 (`captures/evaluation-run4-panel-loop_hands-on_arm-port_2026-09-02/`) ran the same loop through the panel's surfaces after building them: `send --with-capture` (the panel closes itself first so the capture shows the preview and not the panel), `preview --focus` and `open` focusing the preview beside the terminal, `p` and a Preview button on rows with a preview, the loop count leading each row's second line. One instruction went in with the panel's argv over ssh, the second through the panel itself with `wtype` pressing `s`, the text, and Enter; both were delivered with the capture they were about and both produced their commit. The first attempt of the run branched from `main`, which never had `hello.html`; the agent asked where the footer should go and the record said `blocked`, the first unplanned blocked state it has carried and the right one. The legend overflowed at seven entries and now caps at six; the loop view now sorts by the record's sequence inside a second. Tests: 121 here and on the rig.

Still on the rig's screen and not mine to clear: the keyring modal and five stale critical toasts from earlier runs. The loop-panel worktrees (`loop-panel`, `loop-panel2`) sit under `~/.herdr/worktrees/` with the run-3 `loop-hello` one; the spike repo's `session/*` branches hold the commits.

## Addendum, 21:40 to 22:10: Phase 6 drafts

`outbound/` now holds the listing package, the submission text, the discussion post, and six upstream issue texts. `build-listing.sh` assembled the listing directory twice, once per candidate id, and `omarchy-plugin-validate` on the rig accepted both; `install.sh` and `uninstall.sh` ran against a throwaway HOME with a shim in place of `systemctl` and behaved as documented (twenty links, two units, refusals for a foreign file and a foreign unit, a clean removal), with the rig's real units untouched and still active. The watcher's Herdr socket now defaults to `~/.config/herdr/herdr.sock`, so the rig's env drop-in is no longer load-bearing; the unit files on the rig were replaced with the repo's, which carry the same ExecStart and a shorter, current comment, and the watch unit was restarted. The style gate ran over the outward-facing texts with the live rules loaded.

## Addendum, 22:00 to 22:20: the name, and a session started from the panel

The plugin is Keepalive (Praneet's pick from five names; `decisions.md`). The listing texts were rewritten in plain language at his request and carry the new name; the dev checkout and the rig keep the `praneet.agent-sessions` id until he says to rename.

He then asked for a way to start a session from the panel. Built and run as run 5 (`captures/evaluation-run5-start-from-panel_…/`): a starter row under the hero, `n` or a click or a right click on the bar icon opens a field, Enter runs `scripts/new-session.sh`, which picks the default agent, creates the session in `~/Work` with the text as the first prompt, and opens its terminal. Over ssh with `wtype`: `n`, a prompt asking for a file, Enter; the session `create-a-file-named-2` existed at 05:11:28Z, its terminal was on screen by 05:11:44Z, and the agent wrote the file 30 seconds after Enter. The test session was stopped and the file it wrote removed. The legend gained "n new" and now takes its six entries in priority order.

## Addendum, 22:20 to 22:35: the marketplace's guides, and the listing id on the rig

Read plugins.omarchy.org's development guide (13 Aug 2026) and publishing guide (20 Aug 2026). The bar-widget contract they document (`BarWidget.qml` loading `Panel.qml`) is one of two the shell supports; the `Panel`-as-entry-point form this plugin uses is the one quattro's own `agents` plugin uses at HEAD, so no restructure. Two things came from the guides: `onTabRequested` now walks to the neighbouring bar panel through `switchPanel` (the Tab item from the review), and the pre-share checklist. PLAN.md Phase 6 is rewritten against the guides.

Run 6 (`captures/evaluation-run6-listing-id-checklist_…/`): the rig switched from `praneet.agent-sessions` to `io.github.mphaxise.keepalive`, installed from the built listing directory, keybindings rebound, then the checklist end to end and a session started from the panel under the new id. The dev checkout's plugin directory is renamed to match. The `/usr/bin/qmllint` wrapper on this image exits 255 silently; `/usr/lib/qt6/bin/qmllint` is the working binary and reports the same warning classes for this plugin as for the built-ins.

## Addendum, 22:30 to 22:45: the reboot regression

Praneet rebooted at 22:30:19 and pressed Enter on a session; it opened the receipt. The reconciler had marked both his working sessions `failed` ("harness exited while working") at 22:30:31, one second after the post-reboot Herdr came up with a fresh shell in every restored workspace. Fixed in the core (a server start newer than the binding means orphaned, not ended; an end inferred that way still revives on `open`), the watcher (restart orphans at normal urgency), and the panel (such rows read "failed · resumes conversation" with Revive and Receipt). 125 tests. Deployed and verified on the rig by reviving my run-2 `failed` session: failed → orphaned → working → idle, terminal open, `--resume` on its transcript, then stopped. His two sessions show Revive; they are his to press. The Herdr-restart path itself has the unit test and not a live run, because restarting the server would have killed his live sessions.

## Addendum, 22:45 to 22:55: published

On "go all four": pushed the dev repository (17 commits, `382c785..ea2bc08`); built the listing again with the maintainer named and created `mphaxise/omarchy-keepalive` from it (`b96ecbc`); on the rig, removed the copied plugin directory and installed from the public URL with `omarchy plugin add --enable --yes` (run 8: enabled, validated, IPC answering, `plugin update` up to date, install and uninstall scripts clean against a throwaway HOME); opened the marketplace submission as `omacom/omarchy-plugin-marketplace#4566`; posted the discussion as `omacom/omarchy#9936`, Show and tell. The rig's plugin directory is now a git checkout of the public repository, so `omarchy plugin update` is the update path there too; development deploys by rsync into it still work.
