...
4. **A scenario error is a blocked session, correctly.** The first attempt branched from `main`, which never had `hello.html`; the agent asked where the footer should go, Herdr reported `blocked`, the row would have said "needs you". The scenario was wrong and the surfaces behaved. It is noted here as the first unplanned `blocked` the record has carried.
5. **Feedback labels count captures, not feedback.** `feedback-2` is the second capture on the session, not the second piece of feedback. The number is right and the word is loose. It stays, because the `about` reference is by path and the label is for a person scanning the artifact list.
